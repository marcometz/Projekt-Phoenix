...
// init
//
// Constructs empty output and error string storage.
//
// returns the initialized ScriptStep
//
- (id)init
{
	self = [super init];
...
- (NSAttributedString *)attributedStringForString:(NSString *)aString
{
	...
	return
		[[[NSAttributedString alloc]
			...
			attributes:currentQueue.textAttributes]
		autorelease];
}
...
// title
//
// If no title string is set, return the instance description
//
// returns the string to use for this step's title
//
- (NSString *)title
{
	if (!title)
...
// runStep
//
// Empty in this default case but this is the method to override to perform the
// real work of the step
//
- (void)runStep
{
}
...
- (NSString *)outputString
{
	if (![[NSThread currentThread] isEqual:[NSThread mainThread]])
	{
		...
		[self
			performSelectorOnMainThread:@selector(outputStringIntoArray:)
...
- (NSString *)errorString
{
	if (![[NSThread currentThread] isEqual:[NSThread mainThread]])
	{
		NSMutableArray *container = [NSMutableArray arrayWithCapacity:0];
		[self
			performSelectorOnMainThread:@selector(errorStringIntoArray:)
			withObject:container
			waitUntilDone:YES];
		return [container lastObject];
	}
...
// scriptValueWithKey:
//
// ScriptValues are used by various ScriptSteps. In some cases, instead of
// simple literal strings you can use a ScriptValue instead, which resolves
// itself at runtime from the ScriptQueue state value identified by the stateKey.
//
// Parameters:
//    aStateKey - identifies the queueState value
//
// returns the initialized ScriptValue object
//
